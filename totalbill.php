...
$d=$_POST['date'];
$str="select sum(dish_price) from bill, bill_date where bill_date='$d' and bill.bill_no=bill_date.bill_no group by bill_date";
$result=oci_parse($conn,$str);
oci_execute($result);
$rows=oci_fetch_assoc($result);
echo "Total sales of ".$d." is ".$rows['SUM(DISH_PRICE)'];
}
?>
</body>
</html>
